...
impl Runtime {
    ...
    pub fn at_position(
        ...
        let Some(store) = self.store.as_ref() else {
            ...
            envelope.status = repin_core::protocol::envelope::Status::Unavailable;
            return envelope;
        };
        let Ok(view) = store.read_view() else {
    ...
    pub fn rebuild(
        ...
        match target {
            ...
            repin_core::protocol::ipc::RebuildTarget::Lexical => self
                .store
                .as_ref()
            ...
            repin_core::protocol::ipc::RebuildTarget::Vector => {
...
fn unavailable_graph(error: Option<&StoreError>) -> ResultEnvelope<Vec<RankedCandidate>> {
    ...
    let code = match error {
        Some(StoreError::SchemaVersionMismatch { found, supported }) if found > supported => {
            repin_core::protocol::errors::ErrorCode::ProjectStateNewer
        }
        Some(_) => repin_core::protocol::errors::ErrorCode::ProjectStateInvalid,
        None => repin_core::protocol::errors::ErrorCode::CapabilityUnavailable,
    };
    let message = error
