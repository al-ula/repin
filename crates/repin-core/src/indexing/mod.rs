...
use crate::ports::fs::{FileSnapshot, SourceError, SourceFs};
use crate::ports::pack::LanguagePack;
use crate::ports::store::{Store, StoreError, UpdateSummary, VersionRecords};

pub mod coordinator;

pub use coordinator::{BlastRadius, IndexingCoordinator, InvalidationScope};

/// Historical alias for the indexing coordinator.
...
mod tests {
    use super::*;
    use crate::fs::CapabilityFs;
    use crate::packs::default_packs;
    use crate::ports::pack::{ExtractedFacts, ExtractionError, LanguagePack};
    ...
    fn classification_replacement_updates_persisted_claims_without_source_reads() {
        ...
        IndexingCoordinator::reclassify_files(
            ...
            |_| Some(crate::model::registries::ArtifactClass::Docs),
            &records,
        )
        .unwrap();
        let view = store.read_view().unwrap();
        assert!(
            view.nodes_by_file("root", "lib.rs")
                ...
                .all(|node| node.artifact_class
                    == Some(crate::model::registries::ArtifactClass::Docs))
        );
    }
}
