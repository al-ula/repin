...
//! reads use [`repin_core::ports::SourceFs`], while graph enrichment uses the
//! borrowed [`repin_core::ports::ReadView`] contract.

use repin_core::config::ContextConfig;
use repin_core::model::identity::NodeId;
use repin_core::model::node::Node;
use repin_core::ports::fs::SourceFs;
...
mod tests {
    ...
    use repin_core::ports::fs::{FileSnapshot, SourceError};
