use repin_core::line_index::{ByteSpan, LineIndex, Range};
...
pub fn node_text<'a>(node: &TsNode<'_>, source: &'a [u8]) -> &'a str {
...
pub fn node_range(node: &TsNode<'_>, line_index: &LineIndex, source: &[u8]) -> Option<Range> {
    let start = node.start_byte();
    let end = node.end_byte();
    if start <= end && end <= source.len() {
        let span = ByteSpan::new(start, end);
        line_index.span_to_range(source, span).ok()
    } else {
        None
    }
}
...
impl<'a> FactBuilder<'a> {
    ...
    pub fn make_node(
        ...
        let range = node_range(ts_node, self.line_index, self.source);
