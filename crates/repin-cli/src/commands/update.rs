...
use repin_product::{GITHUB_API_LATEST_RELEASE, GITHUB_BASE};
...
fn find_extracted_binary(extract_dir: &Path) -> Option<PathBuf> {
    let direct_bin = extract_dir.join(repin_product::BINARY_NAME);
    ...
    if let Ok(entries) = fs::read_dir(extract_dir) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.is_dir() {
                let candidate = path.join(repin_product::BINARY_NAME);
