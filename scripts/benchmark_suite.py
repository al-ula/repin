...
class BenchmarkSuite:
    def __init__(self, repo_dir: str, release_build: bool = True):
        ...
        target_dir = os.environ.get(
            "CARGO_TARGET_DIR", os.path.join(self.repo_dir, "target")
        )
    ...
    def _detect_engines(self) -> dict[str, bool]:
        engines = {
            "repin": os.path.exists(self.repin_bin)
            or shutil.which("repin") is not None,
    ...
    def run_cold_indexing(self) -> dict[str, Any]:
        ...
        if self.available_engines.get("repin") and os.path.exists(self.repin_bin):
            ...
            print(
                f"     Repin (Code-Only): {total_time:.3f}s | Storage: {size / 1024:.1f} KB"
            )
            ...
            subprocess.run(
                "pkill -f 'repin daemon' || true", shell=True, capture_output=True
            )
            import time

            time.sleep(0.2)
            t_init_full = run_cmd(
                f"{self.repin_bin} init --no-index", cwd=self.repo_dir
            )
            ...
            print(
                f"     Repin (Full-Corpus): {total_time_full:.3f}s | Storage: {size_full / 1024:.1f} KB"
            )
        ...
        if self.available_engines.get("codegraph"):
            ...
            print(
                f"     CodeGraph: {cg_init['duration_sec']:.3f}s | Storage: {size / 1024:.1f} KB"
            )
        ...
        if self.available_engines.get("graphify"):
            ...
            gf_init = run_cmd(
                "graphify extract . --code-only --cargo", cwd=self.repo_dir
            )
            ...
            print(
                f"     Graphify: {gf_init['duration_sec']:.3f}s | Storage: {size / 1024:.1f} KB | Nodes: {node_count}, Edges: {edge_count}"
            )
    ...
    def run_incremental_update(self) -> dict[str, Any]:
        ...
        if self.available_engines.get("repin") and os.path.exists(self.repin_bin):
            ...
            print(
                f"     Repin Incremental Sync: {r_clean['duration_sec'] * 1000:.2f} ms"
            )
        return results
    ...
    def run_search_modalities(self, target_symbol: str = "Engine") -> dict[str, Any]:
        ...
        if self.available_engines.get("repin") and os.path.exists(self.repin_bin):
            ...
            r_lex = run_cmd(
                f"{self.repin_bin} search {target_symbol} --limit 10", cwd=self.repo_dir
            )
            ...
            r_hyb = run_cmd(
                f"{self.repin_bin} search --hybrid {target_symbol} --limit 10",
                cwd=self.repo_dir,
            )
            ...
            r_graph = run_cmd(
                f"{self.repin_bin} search --graph {target_symbol} --limit 10",
                cwd=self.repo_dir,
            )
            ...
            r_regex = run_cmd(
                f"{self.repin_bin} search --regex {target_symbol}", cwd=self.repo_dir
            )
    ...
    def run_graph_and_ast_queries(
        self,
        target_symbol: str = "Engine",
        sample_file: str = "crates/repin-core/src/runtime/engine.rs",
    ) -> dict[str, Any]:
        print(
            "\n[4/5] Benchmarking Graph Traversal, AST Inspection & Context Assembly..."
        )
        results = {"graph_traversal": {}, "ast_inspection": {}, "context_assembly": {}}

        # 1. Graph Traversal
        if self.available_engines.get("repin") and os.path.exists(self.repin_bin):
            r_neighbors = run_cmd(
                f"{self.repin_bin} neighbors {target_symbol}", cwd=self.repo_dir
            )
            r_entity = run_cmd(
                f"{self.repin_bin} entity {target_symbol}", cwd=self.repo_dir
            )
            r_impact_1 = run_cmd(
                f"{self.repin_bin} impact {target_symbol} --max-depth 1",
                cwd=self.repo_dir,
            )
            r_impact_3 = run_cmd(
                f"{self.repin_bin} impact {target_symbol} --max-depth 3",
                cwd=self.repo_dir,
            )
            r_path = run_cmd(
                f"{self.repin_bin} path crates/repin/src/main.rs DaemonClient",
                cwd=self.repo_dir,
            )
        ...
        if self.available_engines.get("repin") and os.path.exists(self.repin_bin):
            r_inspect = run_cmd(
                f"{self.repin_bin} inspect {sample_file}", cwd=self.repo_dir
            )
            r_pos = run_cmd(
                f"{self.repin_bin} at-position {sample_file} 35 10", cwd=self.repo_dir
            )
        ...
        if self.available_engines.get("repin") and os.path.exists(self.repin_bin):
            r_ctx = run_cmd(
                f"{self.repin_bin} context {target_symbol}", cwd=self.repo_dir
            )
            r_rev = run_cmd(
                f"{self.repin_bin} review-context --budget 4096", cwd=self.repo_dir
            )
    ...
    def run_eval_harness(self) -> dict[str, Any]:
        ...
        if self.available_engines.get("repin") and os.path.exists(self.repin_bin):
            ...
            print(
                f"     Repin Eval: P@1={results.get('precision_at_1', 'N/A')}, P@5={results.get('precision_at_5', 'N/A')}, MRR={results.get('mrr', 'N/A')}"
            )
        return results
    ...
    def generate_report(
        ...
        md.append(
            "| Engine / Scope | Cold Index Time | Incremental Update | Storage Footprint | Primary Backend |"
        )
        md.append("| :--- | :--- | :--- | :--- | :--- |")
        up_ms = (
            f"{update_res.get('repin_update_clean', {}).get('time_ms', 0):.1f} ms"
            if "repin_update_clean" in update_res
            else "N/A"
        )
        if "repin_code_only" in indexing_res:
            r = indexing_res["repin_code_only"]
            md.append(
                f"| **Repin (Code-Only / Rust)** | **{r['time_sec']:.3f} s** | **{up_ms}** | **{r['storage_kb']:.1f} KB** | Native Rust + SQLite FTS5 |"
            )
        if "codegraph" in indexing_res:
            c = indexing_res["codegraph"]
            md.append(
                f"| **CodeGraph (Rust-Only)** | {c['time_sec']:.3f} s | — | {c['storage_kb']:.1f} KB | AST Indexer + SQLite WAL |"
            )
        if "graphify" in indexing_res:
            g = indexing_res["graphify"]
            md.append(
                f"| **Graphify (AST + Cargo)** | {g['time_sec']:.3f} s | — | {g['storage_kb']:.1f} KB | AST + Semantic JSON Graph |"
            )
        if "repin_full" in indexing_res:
            rf = indexing_res["repin_full"]
            md.append(
                f"| **Repin (Full-Corpus: Code + Specs)** | **{rf['time_sec']:.3f} s** | **{up_ms}** | **{rf['storage_kb']:.1f} KB** | Native Rust + SQLite FTS5 + Schema Interning |"
            )

        # 2. Cross-Engine Search & Retrieval Comparison
        md.append("\n## 2. Cross-Engine Search & Retrieval Comparison\n")
        md.append(
            "| Engine & Search Mode | Command | Latency (ms) | Est. Tokens Injected | Ranking / Search Strategy |"
        )
        md.append("| :--- | :--- | :--- | :--- | :--- |")
        if "repin_search_hybrid" in search_res:
            s = search_res["repin_search_hybrid"]
            md.append(
                f"| **Repin (Hybrid Rank Fusion)** | `repin search --hybrid --limit 10` | {s['time_ms']:.1f} ms | ~{s['tokens']} tokens | Reciprocal Rank Fusion (FTS5 + Graph Degree) |"
            )
        if "repin_search_default" in search_res:
            s = search_res["repin_search_default"]
            md.append(
                f"| **Repin (Lexical FTS5)** | `repin search --limit 10` | {s['time_ms']:.1f} ms | ~{s['tokens']} tokens | Direct SQLite FTS5 Porter Stemming |"
            )
        if "repin_search_graph" in search_res:
            s = search_res["repin_search_graph"]
            md.append(
                f"| **Repin (Symbol Graph)** | `repin search --graph --limit 10` | {s['time_ms']:.1f} ms | ~{s['tokens']} tokens | Exact & Substring AST Symbol Index |"
            )
        if "repin_search_regex" in search_res:
            s = search_res["repin_search_regex"]
            md.append(
                f"| **Repin (Direct Regex)** | `repin search --regex` | {s['time_ms']:.1f} ms | ~{s['tokens']} tokens | Indexless Parallel Worktree Ripgrep Scan |"
            )
        if "codegraph_query" in search_res:
            s = search_res["codegraph_query"]
            md.append(
                f"| **CodeGraph (Symbol Query)** | `codegraph query` | {s['time_ms']:.1f} ms | ~{s['tokens']} tokens | AST Symbol Name Index Search (Limit 10) |"
            )
        if "graphify_query" in search_res:
            s = search_res["graphify_query"]
            md.append(
                f"| **Graphify (Graph Query)** | `graphify query` | {s['time_ms']:.1f} ms | ~{s['tokens']} tokens | BFS Subgraph Traversal Query |"
            )

        # 3. Graph Traversal & AST Inspection
        md.append("\n## 3. Graph Traversal & Structural Inspection\n")
        md.append(
            "| Operation | Engine & Command | Latency (ms) | Est. Tokens Injected | Focus |"
        )
        md.append("| :--- | :--- | :--- | :--- | :--- |")
        ...
        if "repin_neighbors" in gt:
            md.append(
                f"| **In/Out Relations** | Repin (`neighbors`) | {gt['repin_neighbors']['time_ms']:.1f} ms | ~{gt['repin_neighbors']['tokens']} tokens | Upstream callers & downstream dependencies |"
            )
        if "repin_entity" in gt:
            md.append(
                f"| **AST Entity Lookup** | Repin (`entity`) | {gt['repin_entity']['time_ms']:.1f} ms | ~{gt['repin_entity']['tokens']} tokens | AST range, ID, qualified name & attributes |"
            )
        if "repin_impact_1" in gt:
            md.append(
                f"| **Direct Impact (1-Hop)** | Repin (`impact --max-depth 1`) | {gt['repin_impact_1']['time_ms']:.1f} ms | ~{gt['repin_impact_1']['tokens']} tokens | Direct 1-hop upstream affected callers (ADR-025) |"
            )
        if "repin_impact_3" in gt:
            md.append(
                f"| **Blast Radius (3-Hop)** | Repin (`impact --max-depth 3`) | {gt['repin_impact_3']['time_ms']:.1f} ms | ~{gt['repin_impact_3']['tokens']} tokens | Transitive upstream blast radius propagation (ADR-025) |"
            )
        if "repin_path" in gt:
            md.append(
                f"| **Shortest Path** | Repin (`path`) | {gt['repin_path']['time_ms']:.1f} ms | ~{gt['repin_path']['tokens']} tokens | Direct & indirect dependency chain trace (ADR-025) |"
            )
        if "codegraph_callers" in gt:
            md.append(
                f"| **Caller Chain** | CodeGraph (`callers`) | {gt['codegraph_callers']['time_ms']:.1f} ms | ~{gt['codegraph_callers']['tokens']} tokens | Direct call sites |"
            )
        if "codegraph_impact" in gt:
            md.append(
                f"| **Direct Impact (1-Hop)** | CodeGraph (`impact`) | {gt['codegraph_impact']['time_ms']:.1f} ms | ~{gt['codegraph_impact']['tokens']} tokens | Downstream impact propagation |"
            )
        if "codegraph_explore" in gt:
            md.append(
                f"| **Code Exploration** | CodeGraph (`explore`) | {gt['codegraph_explore']['time_ms']:.1f} ms | ~{gt['codegraph_explore']['tokens']} tokens | Verbatim line-numbered source + callers |"
            )
        if "graphify_explain" in gt:
            md.append(
                f"| **Entity Explanation** | Graphify (`explain`) | {gt['graphify_explain']['time_ms']:.1f} ms | ~{gt['graphify_explain']['tokens']} tokens | Community node cluster summary |"
            )
        if "graphify_god_nodes" in gt:
            md.append(
                f"| **Hub Analysis** | Graphify (`god-nodes`) | {gt['graphify_god_nodes']['time_ms']:.1f} ms | ~{gt['graphify_god_nodes']['tokens']} tokens | Top connected hub entities |"
            )

        ast = query_res.get("ast_inspection", {})
        if "repin_inspect" in ast:
            md.append(
                f"| **File Outline AST** | Repin (`inspect`) | {ast['repin_inspect']['time_ms']:.1f} ms | ~{ast['repin_inspect']['tokens']} tokens | Tree-sitter file outline extraction |"
            )
        if "repin_at_position" in ast:
            md.append(
                f"| **Coordinate Resolve** | Repin (`at-position`) | {ast['repin_at_position']['time_ms']:.1f} ms | ~{ast['repin_at_position']['tokens']} tokens | Coordinate line/col enclosing symbol |"
            )

        # 4. Context Assembly & Review Intelligence
        md.append("\n## 4. Context Assembly & Review Intelligence\n")
        md.append(
            "| Strategy | Engine & Command | Latency (ms) | Est. Tokens Injected | Context Packing Mechanism |"
        )
        md.append("| :--- | :--- | :--- | :--- | :--- |")
        ctx = query_res.get("context_assembly", {})
        if "repin_context" in ctx:
            md.append(
                f"| **Query Context** | Repin (`context`) | {ctx['repin_context']['time_ms']:.1f} ms | ~{ctx['repin_context']['tokens']} tokens | Token-budgeted snippet packing & reranking |"
            )
        if "repin_review_context" in ctx:
            md.append(
                f"| **Review Context (ADR-016)** | Repin (`review-context`) | {ctx['repin_review_context']['time_ms']:.1f} ms | ~{ctx['repin_review_context']['tokens']} tokens | Changed file diffs + blast radius snippet packing |"
            )
        ...
        if eval_res:
            ...
            md.append(
                f"- **Total Benchmark Queries:** {eval_res.get('total_queries', 'N/A')}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Automated Codebase Intelligence Benchmark Suite"
    )
    parser.add_argument(
        "--repo-dir",
        default=".",
    ...
    parser.add_argument(
        "--no-build", action="store_true", help="Skip release build check"
    )
    parser.add_argument(
        "--no-clean",
        action="store_true",
        help="Do not clean index directories after run",
    )
    ...
    parser.add_argument(
        "--symbol", default="Engine", help="Target symbol to explore (default: Engine)"
    )
    parser.add_argument(
        "--sample-file",
        default="crates/repin-core/src/runtime/engine.rs",
        help="Sample file for AST inspection",
    )
    ...
    query_results = suite.run_graph_and_ast_queries(
        target_symbol=args.symbol, sample_file=args.sample_file
    )
    eval_results = suite.run_eval_harness()

    report_md = suite.generate_report(
        idx_results, update_results, search_results, query_results, eval_results
    )
    print("\n" + report_md + "\n")
